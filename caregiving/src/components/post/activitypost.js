      ...
      <div className="activity-post-wrapper">
        ...
        <div className='activity-post-content'>
          <div className='activity-post-content-left'>
            ...
            <img className='activity-image' src={imgUrl === '' ? process.env.PUBLIC_URL + '/images/default-volunteer.png' : imgUrl} alt='activity' /> 
            </div>
          ...
          <Card.Body>
            ...
            <button disabled={current_timestamp > activity?.endDate}  className={current_timestamp > activity?.endDate? 'volunteer-button expired': 'volunteer-button'} onClick={handleVolunteerButton}>{current_timestamp < activity?.endDate? activity?.participants?.includes(user.userId) ? 'Joined' : 'Volunteer' : 'Ended' }</button>
                <Button variant='success' onClick={() => setShow(true)}>Add Feedback</Button>
            </div>
        ...
        <Modal show={show} onHide={() => setShow(false)}>
          ...
          <Form.Group as={Col} controlId="formGridName" >
              ...
              <Form.Control name='name' type="text" placeholder="Enter your feedback" onChange={e => setFeedback(e.target.value)} required/>
            </Form.Group>
          </Modal.Body>
          <Modal.Footer>
       ...
       {isSuccess ? <ModalMessage message={'Your feedback has been submitted'} isSuccess={isSuccess} setIsSuccess={setIsSuccess} />: ''}
      </div> 
